...
EOF
close SCRIPT;
print STDERR "mnsubmit $partition_arg -c $priority $name.cmd\n";
my @r = split " ",`mnsubmit $partition_arg -c $priority $name.cmd`;
print STDERR $r[3],"\n";
print $r[3],"\n";
if ($wait) {
  my $jobid = $r[3];
  ...
  while ($notdone) {
    $notdone = 0;
    sleep 30;
    open SQ,"/opt/perf/bin/squeue |" or die "couldn't open in pipe squeue!\n";
    while (<SQ>) {
      my @result = split;
