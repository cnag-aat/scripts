...
foreach my $transcript (@sorted_transcripts) {
    ...
    $transcript->{gff}->[8] = lc_tags($transcript->{gff}->[8]);
    ...
    my $prot_desc = 0;
    my $go = 0;
    if($transcript->{gff}->[8]=~/prot_desc=([^;]+)/){$prot_desc = $1;}
    if($transcript->{gff}->[8]=~/Ontology_term=([^;]+)/){$go = $1;}
    #$transcript->{gff}->[8].=";product=$newprotid";
    $transcript->{gff}->[8].=";product=hypothetical protein";
    ...
    foreach my $gffrec (sort gffsort @{$transcript->{cds}}) {
        ...
        $gffrec->[8].=";prot_desc=$prot_desc" if $prot_desc;
        $gffrec->[8].=";Ontology_term=$go" if $go;
