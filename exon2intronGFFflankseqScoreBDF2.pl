#!/usr/bin/env perl
#use lib "$ENV{HOME}/myperlmods/bioperl/bioperl-live/";
use lib "$ENV{HOME}/myperlmods/";
use lib "~talioto/myperlmods/";
use Bio::Tools::GFF;
use Getopt::Long;
use Bio::SeqIO;
use Bio::Seq;
...
my $introngff;
my $fafile = 0;
my $param = "~talioto/param/human.101007.scoring.param";
my $exec ="~talioto/bin/nextgeneid";
my $in = "-";
my $species = 0;
my $fa = "/project/devel/databases/genomes/hg19/H.sapiens.genome.hg19.main.fa";
GetOptions(
	   'i|input:s'   => \$in,
	   'v:s'       => \$version,
	   'ov:s'      =>$oversion,
...
# open LEN, "get-genome -d $gmapdb -L|" or die $!;
# while (<LEN>) {
#     my @f = split;
#     $chrlen{$f[0]}=$f[2];
# }
...
foreach my $t (keys %transcripts) {
	...
	if ($exon_number == 1) {
	    ...
	    #$newintron->add_tag_value('species',$gmapdb);
