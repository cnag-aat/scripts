...
my ($ntsn,$nt_SNTP,@resto) = split " ",`coverageBed -a $pbase.exon4eval.uniq.projected.bed -b $eabase.exon4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
my ($ntsn_all,$nt_SPTP,@resto2) = split " ",`coverageBed -a $pbase.exon4eval.uniq.projected.bed -b $abase.exon4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
my $nt_pred_total = 0;
open(BED,"<$pbase.exon4eval.uniq.projected.bed") or die $!;while(<BED>){chomp;my @f=split "\t",$_;$nt_pred_total+=($f[2]-$f[1]);}close BED;
my $ntsp = $nt_SPTP/$nt_pred_total;
printf ("Nt_Sn\t%4.1f\n",100*$ntsn);
printf ("Nt_Sp\t%4.1f\n",100*$ntsp);
printf ("Nt_SnSp\t%4.1f\n",100*($ntsn+$ntsp)/2);
unlink "$pbase.exon4eval.uniq.projected.bed";

my ($cdsntsn,$cdsnt_SNTP,@restp) = split " ",`coverageBed -a $pbase.cds4eval.uniq.projected.bed -b $eabase.cds4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
my ($cdsntsn_all,$cdsnt_SPTP,@restp2) = split " ",`coverageBed -a $pbase.cds4eval.uniq.projected.bed -b $abase.cds4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
...
my $cdsntsp = $cdsnt_SPTP/$cdsnt_pred_total;
printf ("CNt_Sn\t%4.1f\n",100*$cdsntsn);
printf ("CNt_Sp\t%4.1f\n",100*$cdsntsp);
printf ("CNt_SnSp\t%4.1f\n",100*($cdsntsn+$cdsntsp)/2);
unlink "$pbase.cds4eval.uniq.projected.bed";

my ($internalexonntsn,$internalexonnt_SNTP,@restq) = split " ",`coverageBed -a $pbase.internalexon4eval.uniq.projected.bed -b $eabase.internalexon4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
my ($internalexonntsn_all,$internalexonnt_SPTP,@restq2) = split " ",`coverageBed -a $pbase.internalexon4eval.uniq.projected.bed -b $abase.internalexon4eval.uniq.projected.bed | gawk '{total+=\$6;cov+=\$5;}END{print cov/total "\t" cov;}'`;
...
my $internalexonntsp = $internalexonnt_SPTP/$internalexonnt_pred_total;
printf ("IeNt_Sn\t%4.1f\n",100*$internalexonntsn);
printf ("IeNt_Sp\t%4.1f\n",100*$internalexonntsp);
printf ("IeNt_SnSp\t%4.1f\n",100*($internalexonntsn+$internalexonntsp)/2);
...
sub parseFile{
	...
	if (m/^###/) {
	    # this the transcript record separator, when you reach it,
	    # you can store the transcript and go on...
	    #die "Current is undefined!\n" if !defined %current;
	    next if ! scalar %current;
	    $totalnumber++ if !eof;
	    ...
	    %current = ();@{$current{cds}} = ();@{$current{exons}} = ();@{$current{utrs}} = ();@{$current{gff}} = ();
	    $new_transcript = 1;
	} elsif (m/^#/) {
	    #skip comments
	} else {
	    #print STDERR $_;
	    #slurp up mRNA, CDS and exon records
	    if ($new_transcript) {
		%current = (); @{$current{cds}} = (); @{$current{exons}} = ();@{$current{utrs}} = ();@{$current{gff}} = ();
		$current{id}= $idcount++;
		$new_transcript = 0;
	    ...
	    #print STDERR "|",$record{feature},"|\n";
	    if (($record{feature} eq 'mRNA')||($record{feature} eq 'transcript')) {
		$current{transcript}=\%record;
		#die;
	    } else {
		if ($record{feature} ne 'gene') {
		    push @{$current{gff}},[split "\t",$_];
		}
	    }
	    if ($record{feature} eq 'CDS') {
		#die;
		push(@{$current{cds}},\%record);
		#print STDERR "added CDS: ",$record{gffstring},"\n";
	    }
	    if ($record{feature} eq 'exon') {
		#die;
		push(@{$current{exons}},\%record);
		$current{exon_starts}->{$record{start}}++;
		$current{exon_ends}->{$record{end}}++;
	    }
	    if ($record{feature} eq 'UTR') {
		#die;
		push(@{$current{utrs}},\%record);
		$current{utr_starts}->{$record{start}}++;
		$current{utr_ends}->{$record{end}}++;
	    }
	}
    }
    if (scalar %current) {
...
#     print STDERR "unique $category itranscripts: $internalexonunumber\n";
    `sort -u -k1,3 -k6,6 $base.cds4eval.bed > $base.cds4eval.uniq.bed` if !-e "$base.cds4eval.uniq.bed";
    `sort -u -k1,3 -k6,6 $base.exon4eval.bed > $base.exon4eval.uniq.bed` if !-e "$base.exon4eval.uniq.bed";
    `sort -u -k1,3 -k6,6 $base.intron4eval.bed > $base.intron4eval.uniq.bed` if !-e "$base.intron4eval.uniq.bed";
    if($evaluate_splicesites){
	`sort -u -k1,3 -k6,6 $base.donors4eval.bed > $base.donors4eval.uniq.bed` if !-e "$base.donors4eval.uniq.bed";
	`sort -u -k1,3 -k6,6 $base.acceptors4eval.bed > $base.acceptors4eval.uniq.bed` if !-e "$base.acceptors4eval.uniq.bed";
    }
    `sort -u -k1,3 -k6,6 $base.internalexon4eval.bed > $base.internalexon4eval.uniq.bed` if !-e "$base.internalexon4eval.uniq.bed";
