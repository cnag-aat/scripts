...
my $center = "CNAG";
my $description = "";
my $min_scaff_size = 1000;
my $min_contig_size = 100;
my $sp = 0;
my $cp = 0;
my $lookuptable = 1;
my $keep = 0;
...
foreach my $scaff (@scaffolds){
  ...
  foreach my $contig_or_gap (split(/(N+)/i,$seq)){# foreach my $contig_or_gap (split(/(N{$num_n,})/i,$seq))
    ...
    if ($element % 2){
      ...
      if ($gapsize == 200){
	$evidence = "proximity_ligation";$gapsize = 100;$component_type="U";
      }elsif($gapsize == 100){
	$evidence = "paired-ends";$gapsize = 100;$component_type="U";
      }else{
